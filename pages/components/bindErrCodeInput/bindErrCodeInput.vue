<template>
	<view class="body_list">
		...
		<view class="text_right_area_date">
			<picker v-if="warehouse_data_show!=''" :value="index" @change="choStore" :range="warehouse_data_show" range-key="errorCode">
				<view class="uni-input" style="width: 400upx;padding: 3px 5px;font-size: 25upx;">{{warehouse_data_show[index].errorCode}}</view>
			</picker>
		</view>
	</view>
</template>
	...
	export default {
			...
			warehouse_data: {
				handler(new_val, old_val) {
					...
					for (let i = 0; i < new_val.length; i++) {
						...
						arr_list.errorCode = new_val[i].errorCode
						arr.push(arr_list)
					}
		...
		methods: {
			choStore(e) {
				...
				this.$emit('choStore', this.warehouse_data_show[this.index].errorCode)
			},
		},
		onLoad() {}
	}
