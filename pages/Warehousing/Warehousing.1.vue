	...
	<view>
		...
		<view class="warehousing_scan_area">
			...
			<view class="body_list">
				...
				<view class="text_right_area">
					<input class="text_right_input" type="text" :placeholder="scan_e" :focus="scan_input_foc" v-model="scan_input_value_b"
	...
	export default {
		...
		methods: {
			...
			GetMaxAllowInstockQty() {
				...
				uni.request({
					url: that.connect_url + 'api/services/wmspda/PurInstock/GetMaxAllowInstockQty',
					data: {
						MAC: that.MAC,
			...
			Instock() {
				...
				if (that.maxAllowQty > that.scan_input_value_b && that.scan_input_value_b > 0) {
					that.loading_wait_show = true
					uni.request({
						url: that.connect_url + 'api/services/wmspda/PurInstock/Instock',
						data: {
							MAC: that.MAC,
							BillCode: that.billCode,
		...
		onLoad() {
			...
			this.billDate = this.receipts_data.billDate.slice(0, 10).replace('T', '')
