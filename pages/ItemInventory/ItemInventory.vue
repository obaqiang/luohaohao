<template>
	<view>
		...
		<MaterialInventoryModal v-bind:data_a="data_a" v-bind:data_b="data_b" v-bind:data_c="data_c" v-bind:data_d="data_d"
		 ...
		 v-bind:show_modal_body="show_modal_body" v-bind:show_modal_from="show_modal_from"
		 v-bind:PurchaseOrderInquirybodyC_from="PurchaseOrderInquirybodyC_from" v-on:showModalBtn="showModalBtn" v-if="MaterialInventoryModal_show" />
		<alertModal v-bind:alert_modal="alert_modal" v-if="alertModal_show" />
		<view class="common_bot_btn" @tap="SureSubmit">{{i18n.sure_text}}</view>
...
<script>
	...
	export default {
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.success) {
							uni.showToast({
								icon: 'none',
								title: that.i18n.alert_modal_delete,
								duration: 2000
							})
							that.record_modal_show = false
		...
		onLoad() {
			...
			this.order_time = this.i18n.order_time
			this.supplier = this.i18n.supplier
			this.supplier_man = this.i18n.supplier_man
			this.show_modal_header = this.i18n.show_modal_header
			this.alert_modal = this.i18n.alert_modal
			this.alert_modal_delete = this.i18n.alert_modal_delete
			// this.MacInfo();
			let that = this
